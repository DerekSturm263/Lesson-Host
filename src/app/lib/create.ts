...
function generateLearnInstructions(skillPrompt: SkillPrompt, type: InteractionType): string {
  ...
  for (let i = 0; i < skillPrompt.chapters.length; ++i) {
    ...
    - 1 ${type} with 2 sentences of text. The first sentence should tell the user that they'll be learning about ${chapter}. The second sentence should briefly describe ${chapter} in an easy-to-understand way and explain what it's used for.

    - 3-5 ${type}s, each with 2-4 sentences of text. These will be used to teach the user about ${chapter} in an interactive way. Make sure to include relevant equations, formulas, syntaxes, definitions, theorems, etc. that are necessary to fully understand ${chapter}. Each element in this chapter should build on from the previous one to teach the user in small steps.
    
    - 1 ${type} summative assessment with 2 sentences of text. The first sentence should tell the user that now that they know about ${chapter}, it's time to demonstrate their knowledge by completing the assessment. The second sentence should explain the assessment's instructions to the user.
    `;
  }

  // Recap.
  prompt += `
  CHAPTER ${skillPrompt.chapters.length + 2}: Recap

  - 1 ${type} with 2 sentences of text. The first sentence should congratulate the user for finishing the lesson. The second sentence should lead into a quick recap of the lesson, which will be broken into several paragraphs.

  ${skillPrompt.chapters.map((chapter) => {
    return `
    - 1 ${type} with 1 sentence of text. The sentence should should be a quick recap of ${chapter}. Don't go into too much detail, just say enough to keep the information fresh in the user's brain.
    `;
  })}
  `;
